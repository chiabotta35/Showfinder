  ...
  try {
    ...
    for (const point of points) {
      ...
      for (const e of (data._embedded?.events ?? [])) {
        ...
        allShows.push({ id: `tm_${e.id}`, sourceIds: { ticketmaster: e.id }, source: 'ticketmaster', artistName, venue: { id: venue?.id??'unknown', name: venue?.name??'Unknown', city: venue?.city?.name??'', region: venue?.state?.stateCode??'', country: venue?.country?.countryCode??'', latitude: parseFloat(venue?.location?.latitude??'0'), longitude: parseFloat(venue?.location?.longitude??'0'), address: venue?.address?.line1 }, date: e.dates.start.localDate, startTime: e.dates.start.localTime?.slice(0,5), ticketUrl: e.url, imageUrl: img?.url, status: statusMap[e.dates.status.code]??'unknown', priceRange: pr?{min:pr.min,max:pr.max,currency:pr.currency}:undefined, isFestival: e.name.toLowerCase().includes('festival')||e.name.toLowerCase().includes('fest'), isTribute: false })
      }
    }
    return allShows
  } catch { return [] }
